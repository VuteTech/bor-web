...
Polkit rules now support variable conditions via `action.lookup()`, so a rule can match on action variables — for example allowing a specific systemd unit rather than all of them. Also fixed: multiple action IDs in one rule are now correctly joined with `||`.
